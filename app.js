...
app.post("/login/auth",async function(req, res) {
  let email = req.body.email;
   ...
   let user =  await db.all("select * from driver where email = ? and password = ?",email,password);
...
app.get("/signupninja", function(req, res) {
  res.render("signupninja");
});

  app.post("/signupninja",async function(req, res) {
  let driver = await db.run("driver").insertOne(req.body);
...
app.post("/signup",async function(req, res) {
  let user = await db.run("user").insertOne(req.body);
  console.log(req.body);
  res.redirect("/login/customer");
});
